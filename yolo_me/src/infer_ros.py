...
def predict_video(detector, camera_id):
    ...
    index = 1
    rospy.init_node('yolov3-predector',anonymous=False) #初始化目标检测节点
    rospy.Subscriber('/stereo/image', Image_ros, callback) #订阅图像话题
    box_pub = rospy.Publisher('boxs',Path,queue_size=1) #发布检测框
    ...
    while (1):
        ...
        index += 1
        results = detector.predict(frame, FLAGS.threshold) #推理
        box = Path()
        ...
        if len(box_list)>=1 :
            for i in range(0, len(box_list)):
                pose1 = PoseStamped()
                pose1.pose.position.x = box_list[i][0] #class
                pose1.pose.position.y = box_list[i][1]  #confidence

                pose1.pose.orientation.x = box_list[i][2]
                pose1.pose.orientation.y = box_list[i][3] #left_top
                pose1.pose.orientation.z = box_list[i][4]
                pose1.pose.orientation.w = box_list[i][5] #right_bottom
                box.poses.append(pose1)
        box_pub.publish(box)
...
def callback(data):
    global ros_img
    global ros_flag
    # print("fggggggggggg",ros_flag)
    # dtype, n_channels = bridge.encoding_to_cvtype2('bgr8')
    ros_flag = True
    bridge = CvBridge()
    ros_img = bridge.imgmsg_to_cv2(data, "bgr8")
    # cv_img = bridge.imgmsg_to_cv2(data)

    # im22 = bridge.compressed_imgmsg_to_cv2(data)

if __name__ == '__main__':
    ros_img = np.zeros([360,640,3],np.uint8)
    ros_img[:,:,0] = np.ones([360,640])*255
    ros_flag = False


    try:
        # rospy.init_node('yolov3-predector',anonymous=False)
        # img_sub = rospy.Subscriber('/usb_cam/image_raw', Image_ros, callback)
