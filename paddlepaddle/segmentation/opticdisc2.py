...
if __name__ == '__main__':
    win_mode = True
    base_path = 'E:\\datasets\\zhujian'
    
    images_source_dir = 'leftImg8bit'
    annotations_source_dir = 'leftImg8bit\\annotations'

    images_target_dir = 'leftImg8bit'
    annotations_target_dir = 'gtFine'
    ...
    #把图片移动到对应目标位置，把标签也移动到对应位置
    for image_file in images_list:
        old_dir = os.path.join(base_path, images_source_dir, image_file)
        new_dir = os.path.join(base_path, images_target_dir)
        shutil.move(old_dir, new_dir)
    for annotation_file in annotations_list:
        old_dir = os.path.join(base_path, annotations_source_dir, annotation_file)
    ...
    with open(os.path.join(base_path, 'train.txt'), 'wb') as f:
        for image, annotation in zip(train_images, train_annotations):
            line = images_target_dir + seperate + image + ' ' + annotations_target_dir + seperate + annotation 
            line = line + '\n'
            f.write(line.encode())
